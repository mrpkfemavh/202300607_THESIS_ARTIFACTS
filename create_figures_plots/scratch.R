...
colnames(appendix_features_controls) <- c("Start", "End", "Bases", "Product", "C1 Australia", "C14 Alpha", "C29 Delta", "C48 Omicron", "Unique SNPs", "SNP/Kb")
...
glimpse(synthetic_combinations_copies)
glimpse(synthetic_combinations_fractions)

names(synthetic_combinations_fractions)[names(synthetic_combinations_fractions) == 'THESIS_IDENTIFIER'] <- 'Identifier'
names(synthetic_combinations_fractions)[names(synthetic_combinations_fractions) == 'THESIS_SYNTHETIC_COPIES_TOTAL'] <- 'Copies'
names(synthetic_combinations_fractions)[names(synthetic_combinations_fractions) == 'THESIS_SYNTHETIC_PRESENT_C1_AUSTRALIA_MT007544_1_FRACTION'] <- 'MT007544_1'
names(synthetic_combinations_fractions)[names(synthetic_combinations_fractions) == 'THESIS_SYNTHETIC_PRESENT_C2_ANCESTRAL_MN908947_3_FRACTION'] <- 'MN908947_3'
names(synthetic_combinations_fractions)[names(synthetic_combinations_fractions) == 'THESIS_SYNTHETIC_PRESENT_C14_ALPHA_EPI_ISL_710528_FRACTION'] <- 'EPI_ISL_710528'
names(synthetic_combinations_fractions)[names(synthetic_combinations_fractions) == 'THESIS_SYNTHETIC_PRESENT_C29_DELTA_EPI_ISL_2693246_FRACTION'] <- 'EPI_ISL_2693246'
names(synthetic_combinations_fractions)[names(synthetic_combinations_fractions) == 'THESIS_SYNTHETIC_PRESENT_C48_OMICRON_EPI_ISL_6841980_FRACTION'] <- 'EPI_ISL_6841980'
synthetic_combinations_fractions$Copies <- synthetic_combinations_fractions$Copies %>% as.integer()
...
depths_blasted <- vroom("/mnt/bulk_data/bgilot/GitHub/2022_EKUT_THESIS_BIOINFO/data/data_unified/sample_depth_statistics_from_mpileups_gini_cohorted.csv") %>% 
  ...
  rename('STAT_IDENTIFIER' = 'Identifier', 'STAT_DEPTH_MEDIAN' = 'Median_Samblaster', 'STAT_DEPTH_MEAN' = 'Mean_Samblaster', 'STAT_DEPTH_MAX' = 'Max_SamBlaster') %>% 
  arrange(Identifier)

depths_unblasted <- vroom("/mnt/bulk_data/bgilot/GitHub/2022_EKUT_THESIS_BIOINFO/data/data_unified/sample_depth_statistics_from_mpileups_gini_cohorted.csv") %>% filter(STAT_SAMPLE_METHOD == "unblasted") %>% 
  select(STAT_IDENTIFIER , STAT_DEPTH_MEDIAN, STAT_DEPTH_MEAN, STAT_DEPTH_MAX) %>% 
  unique()  %>% 
...
synthetic_mixtures <- left_join(synthetic_combinations_fractions, depths_by_method) %>% select(Identifier,  Median_Samblaster, Max_SamBlaster, Median_DeDup, Max_DeDup, Copies, MT007544_1, MN908947_3, EPI_ISL_710528, EPI_ISL_2693246, EPI_ISL_6841980) %>% arrange(desc(Median_Samblaster), desc(Median_DeDup))

synthetic_mixtures$Identifier <- as.character(map(strsplit(synthetic_mixtures$Identifier, split = "_"), 1))

print(xtable(synthetic_combinations_fractions), include.rownames=FALSE)

print(xtable(synthetic_mixtures), include.rownames=FALSE)


synthetic_combinations_copies <- vroom("/mnt/bulk_data/bgilot/GitHub/2022_EKUT_THESIS_BIOINFO/data/data_unified/synthetic_combinations_copies.csv") 
synthetic_combinations_copies <- synthetic_combinations_copies %>%
  select(THESIS_IDENTIFIER, THESIS_SYNTHETIC_COPIES_TOTAL) %>%
...
synthetic_combinations_copies$Identifier <- as.character(map(strsplit(synthetic_combinations_copies$Identifier, split = "_"), 1))
synthetic_combinations_copies$Viral_Copies <- synthetic_combinations_copies$Viral_Copies %>% as.integer()
glimpse(synthetic_combinations_copies) 
...
glimpse(synthetic_samples_to_xtable)


print(xtable(synthetic_samples_to_xtable), include.rownames=FALSE, digits=4)


synthetic_samples_to_xtable[,5:9] <- synthetic_samples_to_xtable[,5:9] %>% round(6) 
synthetic_samples_to_xtable$Median_blind <- synthetic_samples_to_xtable$Median_blind %>% as.character()
synthetic_samples_to_xtable$Median_aware <- synthetic_samples_to_xtable$Median_aware %>% as.character()
synthetic_samples_to_xtable$'Total Copies' <- synthetic_samples_to_xtable$'C1 Fraction' %>% as.character()

synthetic_samples_to_xtable$'C1 Fraction' <- synthetic_samples_to_xtable$'C1 Fraction' %>% as.character()
synthetic_samples_to_xtable$'C2 Fraction' <- synthetic_samples_to_xtable$'C2 Fraction' %>% as.character()
synthetic_samples_to_xtable$'C14 Fraction' <- synthetic_samples_to_xtable$'C14 Fraction' %>% as.character()
synthetic_samples_to_xtable$'C29 Fraction' <- synthetic_samples_to_xtable$'C29 Fraction' %>% as.character()
synthetic_samples_to_xtable$'C48 Fraction' <- synthetic_samples_to_xtable$'C48 Fraction' %>% as.character()

view(synthetic_samples)

synthetic_samples$`Total Copies`



%>% select(THESIS_IDENTIFIER, THESIS_SYNTHETIC_COPIES_TOTAL, ends_with("_FRACTION")) %>% arrange (THESIS_SYNTHETIC_COPIES_TOTAL, THESIS_SYNTHETIC_PRESENT_C1_AUSTRALIA_MT007544_1_FRACTION, THESIS_SYNTHETIC_PRESENT_C2_ANCESTRAL_MN908947_3_FRACTION, THESIS_SYNTHETIC_PRESENT_C14_ALPHA_EPI_ISL_710528_FRACTION, THESIS_SYNTHETIC_PRESENT_C29_DELTA_EPI_ISL_2693246_FRACTION, THESIS_SYNTHETIC_PRESENT_C48_OMICRON_EPI_ISL_6841980_FRACTION) %>% unique()

synthetic_combinations_copies$THESIS_IDENTIFIER <- as.character(map(strsplit(synthetic_combinations_copies$THESIS_IDENTIFIER, split = "_"), 1))
synthetic_combinations_fractions$THESIS_IDENTIFIER <- as.character(map(strsplit(synthetic_combinations_fractions$THESIS_IDENTIFIER, split = "_"), 1))

synthetic_combinations_copies %>% rename('STAT_IDENTIFIER' = 'Identifier', 'STAT_DEPTH_MEDIAN' = 'Median_Samblaster', 'STAT_DEPTH_MEAN' = 'Mean_Samblaster', 'STAT_DEPTH_MAX' = 'Max_SamBlaster') 

synthetic_combinations_copies %>% 
  rename('Identifier' = 'THESIS_IDENTIFIER') %>%    , 'STAT_DEPTH_MEDIAN' = 'Median_Samblaster', 'STAT_DEPTH_MEAN' = 'Mean_Samblaster', 'STAT_DEPTH_MAX' = 'Max_SamBlaster') 
rename('Identifier' = 'THESIS_IDENTIFIER') %>%
